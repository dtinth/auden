...
test('complete freestyle flow: admin configures scene, audience interacts, presentation displays content', async ({
  ...
  await test.step('Custom HTML/CSS Content: Admin injects content, audience and presentation display it', async () => {
    ...
    await admin.freestyle.setPresentationHTML('<h1 id="custom-title">Welcome to Code in the Dark!</h1>')
    await admin.freestyle.setPresentationCSS('#custom-title { color: #ff6b35; font-family: monospace; text-align: center; }')

    // Presentation: Should display custom content
    await presentation.freestyle.expectCustomContent('Welcome to Code in the Dark!')

    // Admin: Set custom HTML content for audience (different from presentation)
    await admin.freestyle.setAudienceHTML('<p id="audience-msg">Audience: Ready to participate?</p>')
    await admin.freestyle.setAudienceCSS('#audience-msg { color: #4ecdc4; font-weight: bold; }')
  ...
  await test.step('Chat Functionality: Admin enables chat, users send messages, presentation shows chat', async () => {
    ...
    await user1.freestyle.expectChatMessage('Bob', 'Bob here, ready for some coding!')
    
    await user2.freestyle.expectChatMessage('Alice', 'Hello from Alice! 👋')
    await user2.freestyle.expectChatMessage('Bob', 'Bob here, ready for some coding!')

    // Presentation: Should show chat messages
    await presentation.freestyle.expectChatMessage('Alice', 'Hello from Alice! 👋')
    await presentation.freestyle.expectChatMessage('Bob', 'Bob here, ready for some coding!')
  })
  ...
  await test.step('Questions System: Users submit questions, vote on them, admin sees top questions', async () => {
    ...
    await user1.freestyle.submitQuestion('What IDE should I use for JavaScript development?')
    await user2.freestyle.submitQuestion('How do I handle async operations in TypeScript?')

    // Verify questions appear
    await user1.freestyle.expectQuestion('Alice', 'What IDE should I use for JavaScript development?')
    await user2.freestyle.expectQuestion('Bob', 'How do I handle async operations in TypeScript?')

    // Cross-voting: Users vote on each other's questions
    await user1.freestyle.likeQuestion('How do I handle async operations in TypeScript?')
    await user2.freestyle.likeQuestion('What IDE should I use for JavaScript development?')
    await user2.freestyle.likeQuestion('How do I handle async operations in TypeScript?') // Bob likes his own question

    // Verify like counts
    await user1.freestyle.expectQuestionLikes('What IDE should I use for JavaScript development?', 1)
    await user1.freestyle.expectQuestionLikes('How do I handle async operations in TypeScript?', 2)
  ...
  await test.step('Both Mode: Admin enables both chat and questions, users can switch between them', async () => {
    ...
    await presentation.freestyle.expectChatMessage('Alice', 'Now in both mode - chat works!')
    await presentation.freestyle.expectChatMessage('Bob', 'Questions and chat together!')
    ...
    await user1.freestyle.submitQuestion('Can we use both features simultaneously?')

    // Verify the new question appears
    await user1.freestyle.expectQuestion('Alice', 'Can we use both features simultaneously?')
    await user2.freestyle.expectQuestion('Alice', 'Can we use both features simultaneously?')
  })
  ...
  await test.step('Scene switching: Admin changes content, real-time updates across all views', async () => {
    ...
    await admin.freestyle.setPresentationHTML('<div id="final-msg">Thanks for participating in Freestyle mode!</div>')
    await admin.freestyle.setPresentationCSS('#final-msg { font-size: 2em; color: #95e1d3; text-align: center; padding: 50px; }')

    // Presentation: Chat should be hidden, new content should appear
    await presentation.freestyle.expectChatHidden()
    await presentation.freestyle.expectCustomContent('Thanks for participating in Freestyle mode!')

    // Admin: Switch audience back to arbitrary mode with final message
    await admin.freestyle.setAudienceDisplayMode('arbitrary')
    await admin.freestyle.setAudienceHTML('<p id="thanks">Thank you Alice and Bob for the great questions and chat!</p>')

    // Audience: Should see final arbitrary content
    await user1.freestyle.expectArbitraryMode()
    await user1.freestyle.expectCustomContent('Thank you Alice and Bob for the great questions and chat!')
    await user2.freestyle.expectCustomContent('Thank you Alice and Bob for the great questions and chat!')
  })
})
...
test('freestyle real-time synchronization: immediate updates across all user types', async ({
  ...
  await test.step('Real-time content updates', async () => {
    ...
    await presentation.freestyle.waitForChatMessage('Alice', 'Testing real-time sync!')
  })
})